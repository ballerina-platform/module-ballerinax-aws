...
`Region` is a enum covering every region known at build time across the commercial, China, GovCloud, and ISO (government/defense-only) partitions, plus the partition-wide pseudo-regions used by global services (e.g. IAM, CloudFront):
